<template>
    <h2 class="text-center text-2xl text-amber-700 mb-8">Create An Account</h2>
    <form class="space-y-5" @submit.prevent="registerByEmail">
        <base-input
            type="text"
...
const { emailRegister, googleLogin, githubLogin, error } = useAuth()
...
const registerByEmail = async () => {
    try {
        await emailRegister(displayName.value, email.value, password.value)
    } catch (err) {
        console.log(err)
    }
}
